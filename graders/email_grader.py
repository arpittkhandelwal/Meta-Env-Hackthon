...
class EmailGrader:
    def grade(self, step: int, action: Action) -> Dict[str, float]:
        ...
        if step == 0:  # Classification
            ...
            score_breakdown["reasoning"] = 0.3 if len(resp) > 20 else 0.05
        elif step == 1:  # Justification
            score_breakdown["correctness"] = 0.5 if len(resp) > 50 else 0.2
            score_breakdown["reasoning"] = 0.5 if "server" in resp or "down" in resp else 0.05
        elif step == 2:  # Reply
            score_breakdown["tone"] = 0.6 if "hi" in resp or "hello" in resp else 0.05
            score_breakdown["correctness"] = 0.4 if "alice" in resp else 0.05
